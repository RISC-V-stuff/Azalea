...
impl MemoryDevice for SimpleUart {
    fn load(&mut self, addr: u32, _size: AccessSize) -> MemResult<Access> {
        let value = match addr - self.base {
            0x00 => self.backend.read_byte().unwrap_or(0) as u32,
            0x04 => self.backend.rx_ready() as u32,
            _ => 0,
        };
        Ok(Access { value, cycles: 0 })
    }

    fn store(&mut self, addr: u32, _size: AccessSize, value: u32) -> MemResult<u32> {
